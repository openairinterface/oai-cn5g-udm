...
- **Email subject**: `[SEC-VUL-OAI-CN5G-UDM]: "Mention the affected
  functionality here"`.
...
in the
[GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-udm/issues),
...
- **Issue title**: `[AI-SEC-VUL-OAI-CN5G-UDM]: "Mention the affected
  functionality here"`.
- **Issue label**: apply the
  https://github.com/openairinterface/oai-cn5g-udm/labels/security
...
The [openairinterface/oai-cn5g-udm](https://github.com/openairinterface/oai-cn5g-udm)
repository contains the OAI implementation of the 5G Core **Unified Data
Management (UDM)**, together with its configuration templates, container
images, deployment artifacts, and CI tooling. Only the 5G Core is in scope in
this repository.

Issues in the other OAI CN5G network functions (AMF, SMF, UPF, AUSF, UDR, NRF,
...
can be reported here if they are reachable through the UDM. In that case, please
state clearly which component is affected and give the commit ID of the affected
submodule in addition to the UDM commit.

Security reports are in scope when they affect the confidentiality, integrity,
or availability of the UDM running in a documented or reasonably expected
deployment.
...
- Memory corruption, crashes, infinite loops, resource exhaustion, or denial of
  ...
  requests on N8, N10, N13, or N35, or by malformed YAML in the configuration
  file parsed at startup.
- Defects in subscriber identity de-concealment (SIDF). The UDM de-conceals a
  SUCI into a SUPI using the ECIES profiles implemented in
  [src/5gaka/](./src/5gaka), so incorrect or unchecked de-concealment, key or
  profile confusion, oracle behaviour that reveals the SUPI, and crashes on a
  malformed or attacker-chosen SUCI are all in scope (3GPP TS 33.501).
- Defects in authentication vector generation, such as errors in the 5G-AKA
  computation of RAND, AUTN, XRES or XRES*, and KAUSF, in sequence number
  handling and re-synchronisation, or any reuse or predictability that weakens
  subscriber authentication (3GPP TS 33.501).
- Exposure of the home network private keys used for SUCI de-concealment,
  authentication vectors, subscriber identifiers such as SUPI or GPSI, or
  subscription data, through logs, statistics, API responses, or error paths.
- Defects in the implemented UDM service operations (the UE authentication
  endpoints for generating and confirming authentication data, the subscriber
  data management retrieval endpoints, the AMF and SMF registration endpoints,
  and the event exposure subscription endpoints) that allow a consumer to read,
  modify, or delete the data of a subscriber it did not request, or to obtain
  an authentication vector for a subscriber it is not entitled to, without
  appropriate authorisation (3GPP TS 29.503).
...
- Container image or default-configuration issues in [docker/](./docker),
  [etc/](./etc), and [openshift/](./openshift) that expose sensitive services,
  secrets, or the home network private keys shipped in the example
  configuration, disable important security controls, or grant unsafe
  privileges in documented or reasonably expected deployments.
...
- Bugs requiring local admin/root access on the UDM host with no privilege
  boundary crossed.
...
- Vulnerabilities solely caused by third-party projects or dependencies should
  ...
  the UDM deployment or requires a change in this repository, please report it
  here as well.
- Attacks that require a trusted operator role, such as an already-authorised
  AUSF, AMF, or SMF deliberately misconfigured by the deployment owner, unless a
...
- Denial of service that only depends on flooding a plaintext, unprotected
  ...
  deployment, rather than on an UDM software defect.
