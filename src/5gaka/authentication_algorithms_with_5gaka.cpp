...
int Authentication_5gaka::generate_vector(const uint8_t opc[16], uint64_t imsi,
  ...
  comUt::print_buffer("udm_ueau", "generate_vector RAND    : ", vector->rand,
                      16);
  /*
  ...
  comUt::print_buffer("udm_ueau", "generate_vector XRES    : ", vector->xres,
                      8);
  /*
   * AUTN = SQN ^ AK || AMF || MAC
   */
  generate_autn(sqn, ak, amf, mac_a, vector->autn);
  comUt::print_buffer("udm_ueau", "generate_vector AUTN    : ", vector->autn,
                      16);
  derive_kasme(ck, ik, plmn, sqn, ak, vector->kasme);
  comUt::print_buffer("udm_ueau", "generate_vector KASME   : ", vector->kasme,
                      32);
  return 0;
}
