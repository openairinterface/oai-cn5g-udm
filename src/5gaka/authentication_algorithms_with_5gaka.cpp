...
void Authentication_5gaka::ComputeOPc(
  ...
  print_buffer("udm_ueau", "ComputeOPc kP   : ", kP, 16);

  RijndaelEncrypt(opP, opcP);

  print_buffer("udm_ueau", "ComputeOPc opP   : ", opP, 16);
  print_buffer("udm_ueau", "ComputeOPc opcP   : ", opcP, 16);
...
void Authentication_5gaka::derive_kseaf(
  ...
  print_buffer("udm_ueau", "derive_kseaf Kausf", kausf, 32);
  kdf(kausf, 32, S, 3 + netName.size, kseaf, 32);
  print_buffer("udm_ueau", "derive_kseaf Kseaf", kseaf, 32);
}
...
void Authentication_5gaka::derive_kausf(
  ...
  print_buffer("udm_ueau", "derive_kausf key", key, 32);
  kdf(key, 32, S, 11 + netName.size, kausf, 32);
  print_buffer("udm_ueau", "derive_kausf kausf", kausf, 32);
}
...
void Authentication_5gaka::derive_kamf(
  ...
  print_buffer("udm_ueau", "derive_kamf kseaf", kseaf, 32);
  kdf(kseaf, 32, S, 7 + supiLen, kamf, 32);
  print_buffer("udm_ueau", "derive_kamf kamf", kamf, 32);
}
...
void Authentication_5gaka::derive_knas(
  ...
  print_buffer("udm_ueau", "derive_knas kamf", kamf, 32);
  kdf(kamf, 32, S, 7, out, 32);
  // memcpy (knas, &out[31 - 16 + 1], 16);
  for (int i = 0; i < 16; i++) knas[i] = out[16 + i];
  print_buffer("udm_ueau", "derive_knas knas", knas, 16);
...
void Authentication_5gaka::derive_kgnb(
  ...
  *(uint32_t*) (S + 1) = htonl(uplinkCount);
  ...
  print_buffer("udm_ueau", "derive_kgnb kamf", kamf, 32);
  kdf(kamf, 32, S, 10, kgnb, 32);
  print_buffer("udm_ueau", "derive_kgnb kgnb", kgnb, 32);
}
...
int Authentication_5gaka::generate_vector(
  ...
  print_buffer("udm_ueau", "generate_vector MAC_A", mac_a, 8);
  print_buffer("udm_ueau", "generate_vector SQN     : ", sqn, 6);
  print_buffer("udm_ueau", "generate_vector RAND    : ", vector->rand, 16);
  /*
   * Compute XRES, CK, IK, AK
   */
  f2345(opc, key, vector->rand, vector->xres, ck, ik, ak);
  print_buffer("udm_ueau", "generate_vector AK      : ", ak, 6);
  print_buffer("udm_ueau", "generate_vector CK      : ", ck, 16);
  print_buffer("udm_ueau", "generate_vector IK      : ", ik, 16);
  print_buffer("udm_ueau", "generate_vector XRES    : ", vector->xres, 8);
  /*
   * AUTN = SQN ^ AK || AMF || MAC
   */
  generate_autn(sqn, ak, amf, mac_a, vector->autn);
  print_buffer("udm_ueau", "generate_vector AUTN    : ", vector->autn, 16);
  derive_kasme(ck, ik, plmn, sqn, ak, vector->kasme);
  print_buffer("udm_ueau", "generate_vector KASME   : ", vector->kasme, 32);
  return 0;
}
...
uint8_t* Authentication_5gaka::sqn_ms_derive(
  ...
  print_buffer("udm_ueau", "sqn_ms_derive() KEY    : ", key, 16);
  print_buffer("udm_ueau", "sqn_ms_derive() RAND   : ", rand_p, 16);
  print_buffer("udm_ueau", "sqn_ms_derive() AUTS   : ", auts, 14);
  print_buffer("udm_ueau", "sqn_ms_derive() AK     : ", ak, 6);
  print_buffer("udm_ueau", "sqn_ms_derive() SQN_MS : ", sqn_ms, 6);
  print_buffer("udm_ueau", "sqn_ms_derive() MAC_S  : ", mac_s, 8);

  f1star(opc, key, rand_p, sqn_ms, amf, mac_s_computed);
  print_buffer("udm_ueau", "MAC_S +: ", mac_s_computed, 8);
