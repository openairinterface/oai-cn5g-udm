...
bool util::is_pid_file_lock_success(const char* pid_file_name) {
  ...
  if (0 > g_fd_pid_file) {
    Logger::udm_server().error("open filename %s failed %d:%s\n", pid_file_name,
                               errno, strerror(errno));
    return false;
  }

  if (0 > util::lockfile(g_fd_pid_file, F_TLOCK)) {
    Logger::udm_server().error("lockfile filename %s failed %d:%s\n",
  ...
  if (ftruncate(g_fd_pid_file, 0)) {
    Logger::udm_server().error("truncate %s failed %d:%s\n", pid_file_name,
                               errno, strerror(errno));
    close(g_fd_pid_file);
    return false;
  }
  ...
  if ((ssize_t)-1 == write(g_fd_pid_file, pid_dec, strlen(pid_dec))) {
    Logger::udm_server().error("write PID to filename %s failed %d:%s\n",
                               pid_file_name, errno, strerror(errno));
    return false;
  }
  return true;
