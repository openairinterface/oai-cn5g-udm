...
void GenerateAuthDataApiImpl::generate_auth_data(
  ...
  /*
    ...
    &udm_cfg.udr_addr.ipv4_addr))); std::string udr_port =
    std::to_string(udm_cfg.udr_addr.port); std::string remoteUri; std::string
    Method; std::string msgBody; std::string Response;

    nlohmann::json j_ProblemDetails;
    ProblemDetails m_ProblemDetails;

    // UDR GET interface ----- get authentication related
    info-------------------- remoteUri = udr_ip + ":" + udr_port +
    ...
    if (authenticationInfoRequest.resynchronizationInfoIsSet()) {
      ...
      r_sqn = Authentication_5gaka::sqn_ms_derive(opc, key, r_auts, r_rand,
    amf);
      ...
      if (r_sqn) {  // Not NULL (validate auts)
        ...
        int i = 0;
        for (i; i < 6; i++) sqn[i] = r_sqn[i];  // generate first, increase
    later sqn_s = conv::uint8_to_hex_string(sqn, 16);
    ...
    // calculate new
    sqn---------------------------------------------------------- unsigned long
    long sqn_value; std::stringstream s1; s1 << std::hex << sqn_s; s1 >>
    sqn_value;  // hex string to decimal value sqn_value += 32;
    std::stringstream s2;
